...
var currentGameNum = 0;
        ...
        function endGame() {
            if(wins===1) {
                $("#score").text("Nice job, you guessed that the spread was " + slideValue + " which was what the spread was.")
            } else {
                $("#score").text("Oh no, you guessed that the spread was " + slideValue + " but it was actually " + varianceNum + " better luck next time.")
            }
        }
        function next2() {
            myfunction2();
...
function onYouTubeIframeAPIReady2(homeTeam,awayTeam) { 
      console.log(homeTeam + awayTeam);
    $.get("https://www.googleapis.com/youtube/v3/search", {
            part : 'snippet',
            maxResults:2,
